...
let scrollToSection = (number) => {
    ...
    if (window.location.pathname != '/portfolio/') {            // -> sustituir por el nombre del repo de Github. ** ENTRE BARRAS!
        sessionStorage.setItem("sectionToGo", number)
        window.location.pathname = '/portfolio/'               // -> sustituir por el nombre del repo de Github. ** ENTRE BARRAS!

    } else {
        let newScrollHeight = document.querySelectorAll("body > main > section ")[number].offsetTop + 50
